...
export function TestHistory({ courseFilter }: { courseFilter?: Test['course'] } = {}) {
  ...
  const attemptsRaw = getStudentAttempts(currentUser.email);
  const attempts = courseFilter
    ? attemptsRaw.filter(a => tests.find(t => t.id === a.testId)?.course === courseFilter)
    : attemptsRaw;
            ...
            >
              ...
              {isMatchPairType ? (
                <div className="space-y-4">
                  ...
                  {/* Display answer options - use shuffled order (as shown during test) so student's selection matches position */}
                  <div className="space-y-2">
                    {(question.shuffledOptions || ['a', 'b', 'c', 'd'].map(k => ({ text: question[`option${k.toUpperCase()}`], originalKey: k }))).map((opt: { text: string; originalKey: string }, optIdx: number) => {
                      const optionText = opt.text;
                      if (!optionText) return null;

                      // studentAnswerIndex = position student selected during test (0,1,2,3 in shuffled order)
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <span className="font-bold text-gray-600 mr-2">Option {optIdx + 1}:</span>

...
export default function ZoomableImage({
    ...
    const buildImageCandidates = (input: string): string[] => {
        ...
        if (trimmed.includes('drive.google.com')) {
            ...
            const thumbMatch = trimmed.match(/[?&]id=([^&]+)/);
            if (thumbMatch?.[1]) fileId = thumbMatch[1];

            if (!fileId) {
                const dMatch = trimmed.match(/\/d\/([^/]+)/);
                if (dMatch?.[1]) fileId = dMatch[1];
            }

            if (fileId) {
                candidates.push(`https://drive.google.com/uc?export=view&id=${fileId}`);
                candidates.push(`https://drive.google.com/uc?id=${fileId}`);
                candidates.push(`https://drive.usercontent.google.com/uc?id=${fileId}&export=view`);
    ...
    const imageCandidates = buildImageCandidates(src);
        ...
        >
            ...
            >
                {!failed ? (
                    ...
                    <div className="w-full max-w-2xl min-h-40 bg-gray-100 border border-gray-300 rounded flex items-center justify-center text-gray-500 text-sm">
                        Image unavailable
                    </div>
                )}
            </div>

...
import { multiCourseStudents } from '../data/students/multi-course-students';
...
/**
 * Merge duplicate entries into a single user object by credentials.
 * This enables long-term structure like courses: ['foundation', 'nata_2026'].
 */
function mergeAccountsByCredentials(accounts: UserAccount[]): UserAccount[] {
  const map = new Map<string, UserAccount>();

  for (const account of accounts) {
    const email = account.email.trim().toLowerCase();
    const key = `${email}::${account.password}`;
    ...
    if (!existing) {
      map.set(key, {
        ...account,
        email,
        courses: Array.from(new Set(account.courses ?? [])),
      });
      continue;
    }
    ...
    map.set(key, {
      ...existing,
      role: existing.role === 'admin' || account.role === 'admin' ? 'admin' : 'student',
      approved: existing.approved || account.approved,
      courses: Array.from(new Set([...(existing.courses ?? []), ...(account.courses ?? [])])),
    });
  }
...
const initialAccounts: UserAccount[] = mergeAccountsByCredentials(rawAccounts);
...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (currentPath === '/login' || currentPath === '/') {
          if (user.role === 'admin') {
            ...
            // Foundation dashboard can show mixed tests for multi-course users.
  ...
  const login = async (email: string, password: string): Promise<LoginResult> => {
    ...
    if (user.role === 'admin') {
      ...
      // Keep Foundation/JEE as primary landing when user has multiple courses.
      navigate('/dashboard/foundation', { replace: true });
    } else if (user.courses?.includes('advance_2026')) {
      navigate('/dashboard/advance-2026', { replace: true });
    } else if (user.courses?.includes('nata_2026')) {

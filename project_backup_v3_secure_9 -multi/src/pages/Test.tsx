...
export default function Test() {
  ...
  const activeSection = selectedTest?.sections?.find(s => s.questionIndices.includes(currentQuestion));
  const isNata = selectedTest?.category === 'nata';
  const isNataCbt = !!(isNata && selectedTest?.perQuestionTimeSec && activeQuestion?.type !== 'drawing');
  ...
  const handleNumericInput = (val: string) => {
    ...
    const newVal = numericInput + val;
    // Basic validation: only one decimal point, max 10 chars
    if ((val === '.' && numericInput.includes('.')) || newVal.length > 10) return;
  ...
  useEffect(() => {
    // Reset numeric input when moving to a new question
    if (activeQuestion?.type === 'numeric') {
      setNumericInput(answers[currentQuestion]?.toString() || '');
    }
  }, [currentQuestion]);
  ...
  useEffect(() => {
    const saveResults = async () => {
      if (showResults && !resultSaved && currentUser && selectedTest) {
        ...
        const totalMarks = scoreData.totalMarks;
        const totalQuestions = questions.length;
        const maxMarks = totalQuestions * 4;
        const percentageValue = maxMarks > 0 ? ((totalMarks / maxMarks) * 100).toFixed(2) : "0.00";
        ...
        try {
          const { error } = await supabase.from('test_results').insert({
            ...
            // time_taken removed as it's not in the current schema
          });

          if (error) {
            console.error('Error saving result to Supabase:', error);
            // Optional: You could show a UI toast here if you have a toast system
          } else {
            console.log('✅ Result saved to leaderboard');
          }
        } catch (err) {
          console.error('Unexpected error during result save:', err);
        }
      }
    }

    saveResults();
  }, [showResults, currentUser, selectedTest, questions.length, timeLeft, resultSaved, saveTestAttempt, calculateScore]);
  useEffect(() => {
    if (!showResults) {
  ...
  // ====== PRINT & SCREEN RECORDING DETECTION ======
  useEffect(() => {
    if (!testStarted) return;

    // 1. Print Event Detection - beforeprint/afterprint
    const handleBeforePrint = () => {
      showViolationWarning('PRINT ATTEMPT DETECTED - VIOLATION RECORDED');
      setScreenshotBlocked(true);
      // Log to console for debugging
      console.warn('🚨 Print attempt detected and blocked');
    };

    const handleAfterPrint = () => {
      showViolationWarning('PRINT ACTION COMPLETED - VIOLATION LOGGED');
      setScreenshotBlocked(true);
    };
    ...
    if (navigator.mediaDevices && originalGetDisplayMedia) {
      navigator.mediaDevices.getDisplayMedia = async function (constraints?: DisplayMediaStreamOptions) {
        showViolationWarning('SCREEN SHARING/RECORDING DETECTED - VIOLATION RECORDED');
        setScreenshotBlocked(true);
        console.warn('🚨 Screen capture API called - blocked');
        // Still call original but log the violation
    ...
    const handlePiPEnter = () => {
      showViolationWarning('Picture-in-Picture detected - VIOLATION RECORDED');
      setScreenshotBlocked(true);
    };

    // 4. Media recording detection - intercept MediaRecorder
    const OriginalMediaRecorder = window.MediaRecorder;
    if (OriginalMediaRecorder) {
      (window as any).MediaRecorder = class extends OriginalMediaRecorder {
        constructor(stream: MediaStream, options?: MediaRecorderOptions) {
          showViolationWarning('SCREEN RECORDING DETECTED - VIOLATION RECORDED');
          setScreenshotBlocked(true);
          console.warn('🚨 MediaRecorder instantiated - potential recording');
          super(stream, options);
        }
      };
    }

    // 5. Detect DevTools via performance timing (additional layer)
    ...
    const detectDevTools = () => {
      ...
      if (widthThreshold || heightThreshold) {
        if (!devToolsOpen) {
          devToolsOpen = true;
          showViolationWarning('DevTools detected - VIOLATION RECORDED');
          setScreenshotBlocked(true);
        }
      } else {
        devToolsOpen = false;
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        showViolationWarning('Tab switched or window minimized - VIOLATION');
        // INSTANT BLACK SCREEN when tab is switched
        setScreenshotBlocked(true);
      }
    };
    const handleBlur = () => {
      setTabSwitchCount(prev => prev + 1);
      showViolationWarning('Focus lost from test window - VIOLATION');
      // INSTANT BLACK SCREEN when window loses focus
      setScreenshotBlocked(true);
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
  ...
  // Fullscreen enforcement: Desktop/Laptop only (not mobile/tablet)
  useEffect(() => {
    if (!testStarted || testCompleted) return;

    const isDesktop = !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);

    if (!isDesktop) return; // Mobile/tablet: no fullscreen requirement
    ...
    const enterFullscreen = () => {
      ...
      const reqFs = el.requestFullscreen ?? (el as any).webkitRequestFullscreen ?? (el as any).msRequestFullscreen;
      if (reqFs) {
    ...
    const handleFullscreenChange = () => {
      ...
      if (!inFs && testStarted && !testCompleted) {
        setFullscreenExitCount((prev) => prev + 1);
    ...
    const evtName = document.exitFullscreen ? 'fullscreenchange' : (document as any).webkitExitFullscreen ? 'webkitfullscreenchange' : 'MSFullscreenChange';
    enterFullscreen();
    ...
    return () => {
      ...
      if (exitFs && getFullscreenElement()) exitFs.call(document);
    };
  }, [testStarted, testCompleted]);
  ...
  useEffect(() => {
    // Timer handling moved into TestContext (supports NATA per-question timer).
    return;
  ...
  const handleEnterFullscreen = () => {
    // Use containerRef when in quiz, else documentElement (e.g. when on results view)
    const el = containerRef.current || document.documentElement;
    const reqFs = el.requestFullscreen ?? (el as any).webkitRequestFullscreen ?? (el as any).msRequestFullscreen;
    if (reqFs) {
      (reqFs.call(el) as Promise<void>).then(() => {
        setScreenshotBlocked(false);
        setIsFullscreen(true);
      }).catch(() => {
        setScreenshotBlocked(false); // Still dismiss blocker even if fullscreen fails
      });
    } else {
      setScreenshotBlocked(false);
    }
  ...
  const showPerQuestionTimer = selectedTest?.perQuestionTimeSec && perQuestionTimeLeft !== null;
  const showPartATimer =
    isNata &&
    activeQuestion?.type === 'drawing' &&
    selectedTest?.duration &&
    selectedTest.duration >= 90 * 60;

  const getPartATimeLeft = () => {
    if (!showPartATimer || !selectedTest) return 0;
    const totalDuration = selectedTest.duration; // e.g. 180 min
    const partADuration = 90 * 60; // 90 min
    // Time spent so far = totalDuration - timeLeft
    const timeSpent = totalDuration - timeLeft;
    const remaining = partADuration - timeSpent;
    return remaining > 0 ? remaining : 0;
  };

  useEffect(() => {
    // If it's a Dheya test, we allow access without full authentication
    const isDheyaTest = selectedTest?.course === 'dheya';

    if (!isAuthenticated && !isDheyaTest) {
      navigate('/login');
    } else if (currentUser?.role === 'admin' && !testStarted) {
      navigate('/admin');
    }
  }, [isAuthenticated, currentUser, testStarted, navigate, selectedTest]);

  if (!isAuthenticated && selectedTest?.course !== 'dheya') {
    return null;
  }
  ...
  if (!testStarted && !selectedTest) {
    // Filter by user's enrolled courses FIRST
    // Special handling: 'free_tests' users should see 'dheya' tests
    const userCourseTests = tests.filter(test => {
      if (currentUser?.courses?.includes('free_tests') && test.course === 'dheya') {
        return true; // Free test users can access dheya tests
      }
    ...
    const filteredTests = selectedCategory
      ? userCourseTests.filter(t => t.category === selectedCategory)
      : userCourseTests;
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            {testCategories.map(category => (
              <button
                ...
                className={`p-4 rounded-xl border-2 transition transform hover:scale-105 ${selectedCategory === category.id
                        ...
                        <tr key={attempt.id} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`font-bold ${attempt.score >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                              {attempt.score} / {attempt.maxMarks ?? (attempt.totalQuestions * 4)}
                            </span>
                          </td>
  ...
  if (selectedTest && !testStarted) {
    const savedProgress = checkSavedProgress(selectedTest.id);
    const isNata = selectedTest.category === 'nata';
    const hasPartA = !!selectedTest.sections?.some(s => s.type === 'drawing');
                ...
                <>
                  {hasPartA && (
                    <li className="flex items-start">
                      ...
                      Part A (Offline): A1 (25) + A2 (25) + A3 (30) = 80 marks (Manual evaluation)
                    </li>
                  )}
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">{hasPartA ? '4.' : '3.'}</span>
                    Part B: B1 MCQ (+2, 0 negative) • B2 NCQ (+4, 0 negative)
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">{hasPartA ? '5.' : '4.'}</span>
                    Fullscreen mode is required; violations are tracked
                  </li>
                </>
              ) : (
                <>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">3.</span>
                    Aptitude & Math MCQ: +4 for correct, -1 for incorrect
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">4.</span>
                    Math Numeric: +4 for correct, 0 for incorrect (No negative marks)
                  </li>
                  <li className="flex items-start">
                    <span className="text-blue-600 mr-2">5.</span>
                    Drawing: 50 Marks each (Submit via WhatsApp after test)
                  </li>
                  <li className="flex items-start">
          ...
          {savedProgress ? (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <h4 className="font-bold text-orange-800">Unfinished Attempt Found</h4>
                  <p className="text-sm text-orange-700">
                    You have saved progress from {new Date(savedProgress.savedAt).toLocaleString()}.
                    Time remaining: {Math.floor(savedProgress.timeLeft / 60)} min {savedProgress.timeLeft % 60} sec.
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <button
                  onClick={() => {
                    if (window.confirm("Are you sure you want to discard your saved progress and start fresh? This cannot be undone.")) {
                      clearSavedState(selectedTest.id);
                      startTest();
                    }
                  }}
                  className="flex-1 bg-white border-2 border-orange-200 text-orange-700 hover:bg-orange-50 font-bold py-3 rounded-lg transition"
                >
                  Discard & Start Fresh
                </button>
              </div>
  ...
  if (showResults) {
    ...
    // Section-wise statistics for results
    const getSectionStats = () => {
      const sectionsToUse = selectedTest?.sections && selectedTest.sections.length > 0
        ? selectedTest.sections
        : [{ id: 'overall', name: 'Overall', questionIndices: questions.map((_, i) => i) }];
      ...
      const sectionStats = sectionsToUse.map((section) => {
        ...
        section.questionIndices.forEach((idx: number) => {
          ...
          if (isAttempted) {
            attempted++;
            let isCorrect = false;
            if (q.type === 'numeric') isCorrect = String(userAnswer) === String(q.correctOption);
            else if (q.type === 'drawing' || q.correctOption === 'manual_evaluation') isCorrect = false;
            else isCorrect = userAnswer === q.correctIndex;
            if (!isCorrect) incorrectCount++;
          } else {
            skipped++;
          }
        });
        return { name: section.name, attempted, skipped, incorrect: incorrectCount, total: section.questionIndices.length };
      });

      const totals = sectionStats.reduce((acc, s) => ({
    ...
    return (
      <>
        {screenshotBlocked && <ScreenshotBlocker onEnterFullscreen={handleEnterFullscreen} />}
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 p-4">
                ...
                <tbody>
                  {sectionStats.map((s) => (
                    <tr key={s.name} className="border-b border-gray-100 hover:bg-gray-50/50">
                      ...
                      <td className="py-3 px-4 text-center"><span className="font-semibold text-blue-600">{s.attempted}</span></td>
                      <td className="py-3 px-4 text-center"><span className="font-semibold text-gray-600">{s.skipped}</span></td>
                      <td className="py-3 px-4 text-center"><span className="font-semibold text-red-600">{s.incorrect}</span></td>
          ...
          <div className="space-y-4 mb-8">
            {questions.map((q, idx) => {
              ...
              if (q.type === 'numeric') {
                ...
                isCorrect = false; // Manual evaluation
              } else {
              ...
              return (
                <div key={q.id} className={`border-2 rounded-lg p-4 ${isCorrect ? 'border-green-300 bg-green-50' : isAttempted ? 'border-red-300 bg-red-50' : 'border-gray-300 bg-gray-50'}`}>
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                      {isCorrect ? (
                        ...
                        ) : (q.type === 'drawing' || q.correctOption === 'manual_evaluation') ? (
                          <span className="text-blue-600 font-bold text-sm">Manual Review</span>
                        ) : (
                          <>
                            <XCircle className="text-red-600" size={24} />
                  ...
                  <div className="ml-4 space-y-1">
                    {q.type === 'numeric' ? (
                      <>
                        <p className="text-gray-700 text-sm">Your answer: <span className={`font-semibold ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>{isAttempted ? userAnswer : 'N/A'}</span></p>
                        <p className="text-green-600 text-sm font-semibold">Correct: <span className="font-semibold">{q.correctOption}</span></p>
                      </>
                    ) : (q.type === 'drawing' || q.correctOption === 'manual_evaluation') ? (
                      <p className="text-blue-600 text-sm font-semibold">Uploaded for Manual Evaluation</p>
                    ) : (
                      <>
          ...
          <div className="text-center flex gap-4 justify-center">
            <button
              onClick={() => {
                restartTest();
                // Redirect free/dheya tests to free test dashboard
                if (selectedTest?.course === 'dheya' || currentUser?.courses?.includes('free_tests')) {
                  navigate('/free-tests');
                  return;
                }
                // Redirect rules after test:
                // 1) Multi-course (Foundation + NATA) -> Foundation dashboard
                // 2) NATA-only -> NATA dashboard
                // 3) JEE-course-only -> Foundation dashboard
                const courses = currentUser?.courses || [];
                const hasFoundation = courses.includes('foundation');
                const hasNata = courses.includes('nata_2026');
                const hasJeeCourse = hasFoundation || courses.includes('rank_booster') || courses.includes('advance_2026');

                if (hasFoundation) {
                  navigate('/dashboard/foundation');
                } else if (hasNata) {
                  navigate('/dashboard/nata-2026');
                } else if (hasJeeCourse) {
                  navigate('/dashboard/foundation');
                } else if (courses.includes('dheya')) {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-white relative">
      ...
      <div className="bg-white border-b-2 border-gray-200">
        <div className="max-w-full px-4 py-3 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
            {selectedTest?.sections?.map((section) => {
              const isActive = activeSection?.id === section.id;
              const isVisited = section.questionIndices.some(idx => visitedQuestions.has(idx));
              const isDrawingSection = section.type === 'drawing';
              const partATimeLeft = showPartATimer ? getPartATimeLeft() : 0;
              const lockNonDrawingSections = !!(showPartATimer && partATimeLeft > 0);
              const isDisabled = isNata && lockNonDrawingSections && !isDrawingSection;
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  onClick={() => {
                    const firstIdx = section.questionIndices[0];
                    handleQuestionNavigation(firstIdx);
                  }}
                  title={
                    isDisabled
                      ? `Locked until Part A finishes (${formatTime(partATimeLeft)})`
                      : undefined
                  }
                ...
                >
                  {section.name}
                  {isVisited && !isActive && <span className="ml-2 w-2 h-2 bg-blue-400 rounded-full inline-block"></span>}
                </button>
              );
            })}
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              {showPerQuestionTimer ? (
                <div className="flex items-center gap-3">
                  <div className="bg-indigo-600 text-white px-4 py-2 rounded-lg border border-indigo-700 shadow-sm">
                    <div className="text-[10px] font-extrabold uppercase tracking-wider opacity-90">
                      Question Timer (108s)
                    </div>
                    <div className="text-xl font-black tabular-nums">
                      {formatTime(perQuestionTimeLeft)}
                    </div>
                  </div>
                  <div className="bg-gray-100 text-gray-800 px-4 py-2 rounded-lg border border-gray-200">
                    <div className="text-[10px] font-extrabold uppercase tracking-wider text-gray-600">
                      Total Timer
                    </div>
              ...
              {showPartATimer && (
                <div className="text-xs font-semibold text-orange-700 bg-orange-50 px-3 py-2 rounded-lg border border-orange-200">
                  Part A (Offline) Time Left: {formatTime(getPartATimeLeft())}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flex h-[calc(100vh-70px)]">
        <div className="flex-1 overflow-auto">
          <div className="bg-blue-500 text-white px-6 py-3 font-semibold flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {activeSection && (
                <span className="bg-blue-400/50 px-3 py-1 rounded-full text-xs uppercase tracking-wider font-bold border border-blue-300">
                  {activeSection.name}
                </span>
              )}
            </div>
            {activeQuestion.type === 'match-pair' && (
                ...
                <div className="max-w-md mx-auto">
                  <div className="bg-gray-50 border-2 border-gray-200 rounded-lg p-4 mb-4 text-center">
                    <input
                      ...
                      className="text-2xl font-mono font-bold tracking-widest text-blue-600 bg-transparent text-center w-full focus:outline-none"
                    />
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9, '.', 0, 'clear'].map((key) => (
                      <button
                        key={key}
                        onClick={() => handleNumericInput(key.toString())}
                        className={`p-4 text-xl font-bold rounded-lg transition-colors ${key === 'clear'
                  ...
                  <p className="text-sm text-gray-500 mt-4 text-center">
                    Note: Numeric questions have no negative marking.
                  </p>
                </div>
              ) : activeSection?.type === 'drawing' ? (
                <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-8 text-center">
                  <h3 className="text-xl font-bold text-orange-800 mb-4 text-center">Drawing Task</h3>
                  <p className="text-gray-700 mb-6 text-center">
                    Please complete this drawing on your physical sheet.
                    Once the test is over, share a clear photo of your work on WhatsApp.
                  </p>
                  <div className="flex justify-center">
                    <a
                      ...
                      className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-bold transition"
                    >
                      Share on WhatsApp
                    </a>
                  </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 md:right-80 bg-white border-t-2 border-gray-200 p-4 z-50">
            <div className="flex flex-wrap gap-2 md:gap-3 items-center justify-center md:justify-start">
              <button
                ...
                disabled={isNataCbt}
                className={`px-3 md:px-6 py-2 bg-white border-2 rounded font-medium cursor-pointer text-sm md:text-base ${
                  isNataCbt
              ...
              <button
                ...
                disabled={isNataCbt}
                className={`px-4 md:px-8 py-2 rounded font-medium cursor-pointer text-sm md:text-base ${
                  isNataCbt
        ...
        <div className="hidden md:block w-80 bg-gray-50 border-l-2 border-gray-200 overflow-auto">
          ...
          <div className="p-4">
            ...
            {isNataCbt && (
              <div className="mb-3 text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg p-2">
                NATA CBT: Next question unlocks after <span className="font-bold">108 sec</span>. No revisit allowed.
              </div>
            )}

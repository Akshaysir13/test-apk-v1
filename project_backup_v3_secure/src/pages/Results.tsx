...
export default function Results() {
  ...
  const calculateDetailedScore = () => {
    ...
    questions.forEach((q: any) => {
      ...
      if (answerIndex === undefined || answerIndex === -1) return;

      if (q.type === 'numeric') {
        // Numeric questions: exact match, no negative marks
        // Note: For now, numeric logic might need refinement based on how answers are stored
        // but keeping it consistent with existing logic
        if (String(answerIndex) === String(q.correctOption)) {
          correct++;
          totalMarks += 4;
        }
      } else {

...
export default function ScreenshotBlocker({ onEnterFullscreen }: ScreenshotBlockerProps) {
  ...
  useEffect(() => {
    ...
    // Only return true if it's mobile AND NOT in WebView
    setIsMobile((isMobileUA || (isTouchDevice && isSmallScreen)) && !webViewDetected);
  }, []);

  useEffect(() => {
    // Only block interactions on desktop/laptop (not mobile browsers)
    if (isMobile) return;
  ...
  // On mobile browsers and WebView/APK, don't show any blocking screen
  if (isMobile || isWebView) {
    return null;
  }

  // For Desktop/Laptop: Full violation screen with warnings
  return (

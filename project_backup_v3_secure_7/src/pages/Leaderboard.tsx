...
export default function Leaderboard() {
  ...
  const [selectedCourse, setSelectedCourse] = useState<string>(courseContext || 'All Courses');

  // Get the active course filter (use selectedCourse dropdown value)
  const activeCourseFilter = selectedCourse === 'All Courses' ? null : selectedCourse;
  ...
  const fetchAvailableTests = async () => {
    try {
      ...
      if (activeCourseFilter && tests.length > 0) {
        // Normalize course filter for comparison (remove "Course" suffix and lowercase)
        const normalizedCourseFilter = activeCourseFilter.toLowerCase().replace(/\s+course$/i, '').trim();
        console.log('Normalized course filter:', normalizedCourseFilter);

        const courseTestNames = tests
          .filter(t => {
            const normalizedTestCourse = t.course.toLowerCase().replace(/\s+course$/i, '').trim();
            return normalizedTestCourse === normalizedCourseFilter;
          })
          .map(t => t.name);
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      if (selectedTest !== 'all') {
        ...
        // Normalize course filter for comparison
        const normalizedCourseFilter = activeCourseFilter.toLowerCase().replace(/\s+course$/i, '').trim();

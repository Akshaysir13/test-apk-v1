...
export default function Leaderboard() {
  ...
  // Determine course context: prefer navigation state, fallback to user's primary course
  const locationStateCourse = location.state?.course;

  // Logic to determine the primary course from user's enrolled courses if not provided in state
  const getPrimaryCourse = () => {
    if (locationStateCourse) return locationStateCourse;

    if (currentUser?.courses && currentUser.courses.length > 0) {
      if (currentUser.courses.includes('foundation')) return 'foundation';
      if (currentUser.courses.includes('rank_booster')) return 'rank_booster';
      if (currentUser.courses.includes('advance_2026')) return 'advance_2026';
      // Return the first one if no specific priority
      return currentUser.courses[0];
    }
    return 'foundation';
  };

  const [selectedCourse, setSelectedCourse] = useState<string>(() => getPrimaryCourse());
  ...
  }, [currentUser, locationStateCourse]);

  useEffect(() => {
    fetchAvailableTests();
    // When course changes, we might want to reset selectedTest or keep it if 'all'.
    // Typically, when filtering by a new course, the previously selected test might not be valid.
    setSelectedTest('all');
  ...
  const fetchAvailableTests = async () => {
    try {
      ...
      let uniqueTests = [...new Set(data.map((item: any) => item.test_name))];

      // Filter by selectedCourse
      if (selectedCourse && selectedCourse !== 'all' && tests.length > 0) {
        // Normalize course context for comparison (remove "Course" suffix and lowercase)
        const normalizedCourseContext = selectedCourse.toLowerCase().replace(/\s+course$/i, '').trim();
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      // If 'all' tests selected, but we have a course filter, apply it
      if (selectedTest === 'all' && selectedCourse && selectedCourse !== 'all' && tests.length > 0) {
        const normalizedCourseContext = selectedCourse.toLowerCase().replace(/\s+course$/i, '').trim();
        ...
        const filteredData = allData.filter((result: any) =>
          courseTestNames.some(courseTestName =>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-blue-100 text-sm">
                    {selectedCourse !== 'all'
                      ? `Top performers in ${selectedCourse.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}`
          ...
          <div className="p-4 md:p-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-12 text-gray-500">
                ...
                {selectedCourse !== 'all' && <p className="text-sm text-gray-400">Filtering for course: {selectedCourse}</p>}
